...
public:
	...
	std::pair<iterator,iterator> get_pair(const key& k){
		nodelists lists;
		find(k, &lists);
		weak_node_array& currs = lists.first;
		weak_node_array& succs = lists.second;
		shared_node curr = currs[0].lock();
		shared_node succ = succs[0].lock();
		assert(curr.get() != shared_tail.get().get());
		while(succ->marked && succ.get() != shared_tail.get().get()){ succ = succ->next[0];}
		return std::make_pair(iterator(curr),iterator(succ));
	}
